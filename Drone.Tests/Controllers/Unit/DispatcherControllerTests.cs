...
{
    class DroneGateway : IDroneGateway
    {
        public Task<bool> StartRegistration(string droneIpAddress, Guid badgeNumber, string dispatcherUrl, GeoLocation homeLocation) => (this as IDroneGateway).StartRegistration(droneIpAddress, badgeNumber, dispatcherUrl, homeLocation);
        public Task<bool> AssignDelivery(string droneIpAddress, string orderNumber, GeoLocation orderLocation) => (this as IDroneGateway).AssignDelivery(droneIpAddress, orderNumber, orderLocation);
        public Task<bool> OKToSendStatus(string droneIpAddress) => (this as IDroneGateway).OKToSendStatus(droneIpAddress);
    }
...
// Update Status
        ...
        {
            var droneGateway = new DroneGateway();
            var controller = new DispatcherController(droneGateway);
