...
namespace Drone.Tests.Controllers.Unit
{
    public class DispatcherControllerTests
    {
        ...
        private static void SetUpAll(out DispatcherController controller, out Mock<IOrdersRepository> orders, out Mock<IDroneGateway> gateway, params  MethodSetups[] toggles)
        {
            ...
            if(toggles.Contains(MethodSetups.DroneShouldCreateAsync)) drones.Setup(x => x.CreateAsync(It.IsAny<Domain.Entities.Drone>())).Returns<Domain.Entities.Drone>(Task.FromResult);
            if(toggles.Contains(MethodSetups.DroneShouldGetByIdAsync)) drones.Setup(x => x.GetByIdAsync(It.IsAny<string>())).Returns(Task.FromResult(new Domain.Entities.Drone()));
            if(toggles.Contains(MethodSetups.DroneShouldGetAllAvailableDronesAsync))drones.Setup(x => x.GetAllAvailableDronesAsync()).ReturnsAsync(new List<Domain.Entities.Drone>{TestDrone}.AsEnumerable());
            if(toggles.Contains(MethodSetups.OrderShouldGetByIdAsync))orders.Setup(x => x.GetByIdAsync(It.IsAny<string>())).ReturnsAsync(TestOrder).Verifiable();
            if(toggles.Contains(MethodSetups.OrderShouldUpdate))orders.Setup(x => x.Update(TestOrder)).ReturnsAsync(TestOrder).Verifiable();
            if(toggles.Contains(MethodSetups.GatewayShouldStartRegistration))gateway.Setup(x => x.StartRegistration(It.IsAny<string>(), It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<GeoLocation>())).ReturnsAsync(true).Verifiable();
            if(toggles.Contains(MethodSetups.GatewayShouldAssignDelivery2))gateway.Setup(x => x.AssignDelivery(It.IsAny<string>(),It.IsAny<string>(), It.IsAny<GeoLocation>())).Verifiable();
