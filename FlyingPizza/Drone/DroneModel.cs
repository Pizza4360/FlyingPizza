...
namespace FlyingPizza.Drone
{
    ...
    public class DroneModel : ComponentBase
    {
        ...
        private async Task RegisterDrone()
        {
            ...
            var url = "http://localhost:8080/Fleet?filter={badgeNumber:" + BadgeNumber + "}&keys={_id:0,badgeNumber: 1}";
            var entries = await RestSvc.Get<int[]>(url);
            if(entries.Length < 1)
            {
            ...
            var task = RestSvc.Put<DroneModel>(url, this);
